<script>
	...
	$: name = hero.name;
	$: eventTitle = `[S25-RON] ${name} ml ${year} - Mobile Legends: Bang Bang`;
	$: eventDescription = `${name} ml - All you need to know about ${name} skills, skill combo, hero build, item setup, and spells on Mobile Legends Season 25 Rise if Necrokeep - Be Updated and Be Legendary.`;
</script>

<MetaTags
	title={`${eventTitle}`}
	canonical={`${siteUrl}/${name}-ml`}
