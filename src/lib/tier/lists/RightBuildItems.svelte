<script>
  ...
  function handleDndConsider(e) {
    $heroMatchupBoard.right.builds = e.detail.items
  }
...
<section
  ...
  use:dndzone={{
    items: $heroMatchupBoard.right.builds,
    flipDurationMs,
    type: 'items',

    dropFromOthersDisabled: $heroMatchupBoard.right.builds.length < 6 ? false : true
  }}
  on:consider={handleDndConsider}
  on:finalize={handleDndFinalize}
>
  {#each $heroMatchupBoard.right.builds as item (item.id)}
    <div
